...
def prefill(model, cond_idx: torch.Tensor, input_pos: torch.Tensor, cfg_scale: float):
    if cfg_scale > 1.0:
        logits, _ = model(None, cond_idx=cond_idx, input_pos=input_pos)
...
def decode_n_tokens(gpt, cur_token, input_pos, num_new_tokens, cfg_scale):
    new_tokens, new_probs = [], []
    cfg_flag = True
    for i in trange(num_new_tokens):
        with torch.backends.cuda.sdp_kernel(enable_flash=False, enable_mem_efficient=False, enable_math=True): # Actually better for Inductor to codegen attention here
            next_token = decode_one_token(gpt, cur_token, input_pos, cfg_scale)
            input_pos += 1
...
def decode_one_token(gpt, x, input_pos, cfg_scale):
